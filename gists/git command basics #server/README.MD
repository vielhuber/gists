...
- scenario: private repos A and B should be checked out on the same server
- github does not allow the same deploy keys on different repositories
- solution: create and use second key
  - `ssh-keygen -t rsa -f ~/.ssh/id_rsa_repo2`
  - `chmod 600 ~/.ssh/id_rsa_repo2`
  - `cat ~/.ssh/id_rsa_repo2.pub`
  - `cd /path/to/repo2`
  - `git clone -c core.sshCommand="ssh -F /dev/null -i \"$HOME/.ssh/id_rsa_repo2\" -o IdentitiesOnly=yes" git@github.com:xxx/yyy.git .`
  - change existing repository: `git config --local core.sshCommand "ssh -F /dev/null -i \"$HOME/.ssh/id_rsa_repo2\" -o IdentitiesOnly=yes"`
  - check setting: `cat ./.git/config`
...
- `git remote set-url origin https://david%40vielhuber.de@gitlab.com/vielhuber/vielhuber.git`
...
#### git worktree llm workflow

```
# setup
cd /project
git worktree add ./main-worker-1 -b feature/main-worker-1 main
git worktree add ./main-worker-2 -b feature/main-worker-2 main
git worktree add ./main-worker-3 -b feature/main-worker-3 main

# code
cd ./main-worker-1 && code . && cd ..
cd ./main-worker-2 && code . && cd ..
cd ./main-worker-3 && code . && cd ..

# merge
git merge feature/main-worker-1
git merge feature/main-worker-2
git merge feature/main-worker-3

# cleanup
git worktree remove ./main-worker-1
git worktree remove ./main-worker-2
git worktree remove ./main-worker-3
git branch -d feature/main-worker-1 feature/main-worker-2 feature/main-worker-3
```
...
- **feature <= main**: always rebase
- **main <= feature**: always merge
- **git pull on feature without ff**: always rebase
- **git pull on main without ff**: always merge

| merge | rebase |
|---|---|
| `git pull` | `git pull --rebase` |
| `git merge foo` | `git rebase foo` |
...
- `git pull --rebase` (or `git pull` with config `git config --global pull.rebase true`)
...
- `git push origin --delete branch-name && git branch --delete --force branch-name`
...
- `git remote set-url --push origin git@gitlab.com:vielhuber/new.git`
- `git push --mirror`
...
- option 1: same repo
    ...
    - copying the commit-map has to be done after _every_ single command from git filter-repo (you need the `commit-map` files later)
...
#### gitea: server installation

- the following is already default! only check..
...
- `git lfs install`
- `git lfs track "path/to/big/file.zip"`
- `cat .gitattributes`
- Now `push` and `pull` as normal
- Tip: Enable Column "LFS" in SmartGit
...
- clone a git reposiory inside a git repository
- initial clone: `git subtree add --prefix path/to/subfolder git@github.com:vielhuber/xxx.git main --squash`
- pull: `git subtree pull --prefix path/to/subfolder git@github.com:vielhuber/xxx.git main --squash && git push`
- push: `git subtree push --prefix path/to/subfolder git@github.com:vielhuber/xxx.git main`

#### symlink

- `cd ~/external-lib`
- `git clone git@github.com:vielhuber/xxx.git`
- `cd ~/main-repo`
- `ln -s ~/external-lib ./path/to/subfolder/external-lib`
